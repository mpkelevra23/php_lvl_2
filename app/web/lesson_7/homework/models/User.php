...
class User
{
    ...
    public function registration($name, $email, $password)
    {
        ...
        // Выполняем запрос
        Db::getInstance()->run(
            'INSERT INTO lesson_7.users(name, email, password) VALUES (:name, :email, :password)',
            [$name, $email, $password]
        );
    ...
    public function isAdmin()
    {
        if (Db::getInstance()->run(
            'SELECT id FROM lesson_7.user_role WHERE id_user = :id_user AND id_role = 2',
            [self::getUserId()]
        )->fetch()) {
            return true;
        }
    ...
    public function saveLastActions($userId, $last_actions)
    {
        ...
        return Db::getInstance()->run(
            'UPDATE lesson_7.users SET last_actions = :last_actions WHERE id = :userId',
            [$last_actions, $userId]
        )->rowCount();
    }
