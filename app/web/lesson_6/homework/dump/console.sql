...
CREATE OR REPLACE FUNCTION lesson_6.update_basket() RETURNS TRIGGER
AS
$$
BEGIN
...
CREATE TRIGGER tr_update_basket
    ...
    ON lesson_6."order"
    FOR EACH ROW
EXECUTE PROCEDURE lesson_6.update_basket();

-- Удаляем функцию (чтобы удалить функцию, надо сначала удалить зависимый от неё триггер)
DROP FUNCTION lesson_6.add_user_role();

-- Создаём функцию для добавления id пользователя в таблицу user_role ()
CREATE OR REPLACE FUNCTION lesson_6.add_user_role() RETURNS TRIGGER
AS
$$
BEGIN
...
CREATE TRIGGER tr_add_user_role
    ...
    ON lesson_6.users
    FOR EACH ROW
EXECUTE PROCEDURE lesson_6.add_user_role();
